...
codeunit 50002 "Cmtl Prod. Order Status Mgt."
#pragma warning restore AA0215
{
    ...
    var
        ...
        IsHandled: Boolean;
        HasChildExist: Boolean;
        NewProdOrder: Record "Production Order";
        NewToProdOrder: Record "Production Order";
    begin
        IsHandled := false;
        ...
        HasChildExist := false;
        TempFromProdOrder.Reset();
        TempFromProdOrder.DeleteAll();

        if (Rec.Status = Rec.Status::"Firm Planned") AND (Rec.HasChildren()) then begin
            HasChildExist := true;

            NewProdOrder := Rec;
            if HasChildExist then begin
                GetParentChildProdOrders(Rec."No.", ParentChildProdOrder);
                ParentChildProdOrder.MarkedOnly(true);
                //Message('Prod Order count %1', ParentChildProdOrder.Count);
            end;
        end;
        ...
        if ChangeStatusForm.RunModal() = ACTION::Yes then begin
            ...
            if (Rec.Status = Rec.Status::"Firm Planned") AND (NewStatus = NewStatus::Released) then
                if (Rec."Parent Prod Order No." <> '') AND (Rec.Indentation <> 0) then
                    Error(ChangeStatusErr);
            ChangeProdOrderStatus(Rec, NewStatus, NewPostingDate, NewUpdateUnitCost);

            if HasChildExist then begin
                NewToProdOrder := ToProdOrder;
                if ParentChildProdOrder.FindSet() then
                    repeat
                        ChangeProdOrderStatus(ParentChildProdOrder, NewStatus, NewPostingDate, NewUpdateUnitCost);
                    until ParentChildProdOrder.Next() = 0;
            end;
            Commit();
            if HasChildExist then begin
                Rec := NewProdOrder;
                ToProdOrder := NewToProdOrder;
            end;
    ...
    var
        ...
        ParentChildProdOrder: Record "Production Order";
        ToProdOrder: Record "Production Order";
        TempFromProdOrder: Record "Production Order" temporary;
    ...
    begin
        ...
        with FromProdOrder do begin
            ...
            CreateTempProductionOrders(FromProdOrder."No.", ToProdOrder."No.");
            if ToProdOrder."Parent Prod Order No." <> '' then begin
                if TempFromProdOrder.Get(TempFromProdOrder.Status::"Firm Planned", ToProdOrder."Parent Prod Order No.") then
                    ToProdOrder."Parent Prod Order No." := TempFromProdOrder."Parent Prod Order No.";
            end;
            ToProdOrder.Indentation := Indentation;
            ToProdOrder."Presentation Order" := "Presentation Order";
    ...
    local procedure GetParentChildProdOrders(ProdOrdNo: Code[20]; var _ParentChildProdOrder: Record "Production Order")
    var
        ChildProdOrder: Record "Production Order";
    begin
        ChildProdOrder.Reset();
        ChildProdOrder.SetRange(Status, ChildProdOrder.Status::"Firm Planned");
        ChildProdOrder.SetRange("Parent Prod Order No.", ProdOrdNo);
        if ChildProdOrder.FindSet() then begin
            repeat
                if ChildProdOrder.HasChildren() then
                    GetParentChildProdOrders(ChildProdOrder."No.", _ParentChildProdOrder);
                if _ParentChildProdOrder.Get(ChildProdOrder.Status, ChildProdOrder."No.") then
                    _ParentChildProdOrder.Mark(true);
            until ChildProdOrder.Next() = 0;
        end;
    end;

    local procedure CreateTempProductionOrders(FromProdOrderNo: code[20]; ToProderOrderNo: Code[20])
    var
        myInt: Integer;
    begin
        if not TempFromProdOrder.Get(TempFromProdOrder.Status::"Firm Planned", FromProdOrderNo) then begin
            TempFromProdOrder.Init();
            TempFromProdOrder.Status := TempFromProdOrder.Status::"Firm Planned";
            TempFromProdOrder."No." := FromProdOrderNo;
            TempFromProdOrder."Parent Prod Order No." := ToProderOrderNo;
            TempFromProdOrder.Insert();
        end;
    end;
