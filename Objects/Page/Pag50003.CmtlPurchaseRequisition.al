...
page 50003 "Cmtl Purchase Requisition"
#pragma warning restore AA0215
{
    ...
    SourceTableView = WHERE("Document Type" = FILTER(Quote), "Is Requisition" = const(true));
    ...
    layout
    {
        area(content)
        {
            ...
            part(PurchLines; "Cmtl Purchase Req. Subform")
            {
                ApplicationArea = Suite;
    ...
    actions
    {
        area(navigation)
        {
            group("&Requisition")
            {
                ...
                action("Co&mments")
                {
                    ...
                    Caption = 'Co&mments';
                    Image = ViewComments;
                    RunObject = Page "Cmtl Purch. Comment Sheet";
                ...
                action(MakeQuote)
                {
                    ...
                    begin
                        if ApprovalsMgmt.PrePostApprovalCheckPurch(Rec) then
                            CODEUNIT.Run(CODEUNIT::"CmtlPur. Req to Quote (Yes/No)", Rec);
                    end;
                }
                action(MakeOrder)
                {
                    ...
                    Caption = 'Make &Order';
                    Image = MakeOrder;
                    ToolTip = 'Convert the purchase Requisition to a purchase order.';
                    ...
                    begin
                        if ApprovalsMgmt.PrePostApprovalCheckPurch(Rec) then
                            CODEUNIT.Run(CODEUNIT::"CmtlPur. Req to Order (Yes/No)", Rec);
                    end;
                }
            }
        }
        area(Promoted)
        {
            group(Category_Process)
            {
                ...
                actionref(MakeQuote_Promoted; MakeQuote)
                {
                }
            ...
            {
                Caption = 'Requisition', Comment = 'Generated from the PromotedActionCategories property index 6.';
    ...
    var
        ...
        DocType: Option Requisition,"Order",Invoice,"Credit Memo","Blanket Order","Return Order",Reminder,FinChMemo;
    begin
        DocNoVisible := DocumentNoVisibility.PurchaseDocumentNoIsVisible(DocType::Requisition, Rec."No.");
    end;
    ...
    begin
        ...
        LookupContact(Rec."Buy-from Vendor No.", Rec."Buy-from Contact No.", Contact);
        if PAGE.RunModal(0, Contact) = ACTION::LookupOK then
            Rec.Validate("Buy-from Contact No.", Contact."No.");
